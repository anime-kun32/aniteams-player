(function () {
  ...
  var hlsLibrary = 'https://cdn.jsdelivr.net/npm/hls.js@1.2.0/dist/hls.min.js';

  var query = new URLSearchParams(document.location.search);
  var id = query.get('id');
  var category = query.get('category') || 'sub';
  var playerInstance = null; 

  var apiUrl = `https://aniwatch-api-net.vercel.app/api/v2/hianime/episode/sources?animeEpisodeId=${id}&server=hd-2&category=${category}`;
  ...
  function loadPlayer() {
    fetch(apiUrl)
      ...
      .then(data => {
        ...
        if (!videoSource) throw new Error('No HLS source found');

        // Use the proxy URL for the M3U8 source
        var proxyUrl = `https://gogoanime-and-hianime-proxy-ten.vercel.app/m3u8-proxy?url=${encodeURIComponent(videoSource.url)}`;
// Destroy previous player if it exists
if (playerInstance && typeof playerInstance.destroy === 'function') {
  playerInstance.destroy();
}

playerInstance = OPlayer.make('#oplayer', {
   source: { src: proxyUrl, title: 'OPlayer Video' },
        ...
        // Register audio selection in the context menu
        player.context.menu.register([
          {
            ...
            children: [
              {
                name: 'Sub',
                default: category === 'sub',
                value: 'sub'
              },
              {
                name: 'Dub',
                default: category === 'dub',
                value: 'dub'
              }
            ],
            onChange({ value }) {
              query.set('category', value);
              window.location.search = query.toString(); // Refresh the page with the new category
            }
          }
        ]);
      })
  ...
  loadPlayer(); // Initial load of the player
})();
